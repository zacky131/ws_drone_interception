...
Phase 1.1: Oracle Future Target MPC Baseline (mpc_oracle_target).

Uses exact future ground truth target trajectory over the MPC horizon while
preserving pursuer plant, initial conditions, disturbances, constraints,
MPC cost weights, ACADOS settings, and capture condition.
"""
...
from src.prediction.camps.predictors import OracleTargetPredictor
from src.control.acados_wrapper import build_acados_mpc

class MPCOracleTarget(ControllerBase):
    """MPC controller provided with oracle future target trajectory."""
    ...
    def __init__(
        ...
        self.oracle_predictor = OracleTargetPredictor()
    ...
    def set_scenario(self, scenario: Any) -> None:
        ...
        self.oracle_predictor.set_scenario(scenario)
    ...
    def compute_control(
        ...
        horizon = self.oracle_predictor.predict(
        ...
        return cmd, {
            ...
            "estimator": "oracle",
            "predictor": "oracle_future_target"
        }
